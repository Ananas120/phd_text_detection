...
def UploadBuffer(dicom):
    ...
    if (isinstance(info, dict) and
        ...
        DownloadPreview(info['ID'])
            
def DownloadPreview(instanceID):
    headers = {'Accept': 'image/jpeg'}
    url = 'http://localhost:8042/instances/' + instanceID + '/preview'
    response = requests.get(url, headers=headers)

    if response.status_code == 200:
        with open('/mnt/hgfs/shared/original_images/' + instanceID + '.jpg', 'wb') as f:
            ...
            for name in files:
                if name.endswith('.dcm'):
                    ...
                    if file_size < smallest_file_size:
                        smallest_file = {'root': root, 'name': name}
                        smallest_file_size = file_size
                        if smallest_file is not None:
                            # Print the path of the file with the lowest size in each directory
                            processFile(smallest_file['root'], smallest_file['name'])
                            i += 1
                            if i%5 == 0: #reduce memory usage
                                clear()
    else:
