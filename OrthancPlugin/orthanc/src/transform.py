...
class RandomRotate90(object):
    ...
    def __call__(self, img, bboxes):
        if random.random() < self.prob:
            k = random.randint(1,4) # number of clockwise 90° rotations
            ...
            for _ in range(k):
                ...
                img90 = img_tmp.rotate(270) #90° clockwise
...
class SSDTransformer(object):
    def __init__(self, dboxes, size=(300, 300), normalize_mode="normal", val=False):
        self.size = size
        self.val = val
        self.dboxes = dboxes
        ...
        self.rotate90 = RandomRotate90()
        if self.normalize_mode == "normal":
    ...
    def __call__(self, img, img_size, bboxes=None, labels=None, max_num=200):        
        if self.val:
            ...
            bbox_out, label_out = self.encoder.encode(bboxes, labels)
            return self.trans_val(img), img_size, bbox_out, label_out
        
        img, bboxes = self.rotate90(img, bboxes)
